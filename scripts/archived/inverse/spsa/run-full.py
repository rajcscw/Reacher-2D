...
from scripts.archived.loss import InverseKinematicFull
...
class Linear(nn.Module):
    ...
    def forward(self, input):
        ...
        input = self.layer_4(input)
        input = self.layer_5(input)
        input = self.layer_6(input)
        output = self.layer_7(input)
        return output
...
if __name__ == "__main__":
    ...
    with open(dir_path+"/config_full.yml", 'r') as ymlfile:
    ...
    for k in range(int(config["log"]["runs"])):
        ...
        if config["model"]["is_rec"]:
            net = RNN(input_size=11, output_size=3, hidden_size=config["model"]["hidden_size"])
        else:
            net = Linear(input_size=11, output_size=3, hidden_size=config["model"]["hidden_size"])

        # set up the model
        model = PyTorchModel(net=net, config=config,  device=device, strategy=SamplingStrategy.TOP_DOWN)

        # the objective function
        objective = InverseKinematicFull(model=model,
                                         config=config,
                                         device=device
                                        )
